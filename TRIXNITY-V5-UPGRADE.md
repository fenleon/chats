...
## 8. Post-implementation corrections (2026-08-29, verified against the actual build)

Symbols that differed from the map when the upgrade was executed on the real tree:

- **`crypto.olm` does NOT move to `client.crypto.olm`** (§7 step 2 claim). In 5.8.0 the olm services stay in `de.connect2x.trixnity.crypto.olm` and crypto keys in `de.connect2x.trixnity.crypto.key` — the rename is group-only (`net.folivo` → `de.connect2x`). The `OlmEncryptionService` interface rewrite (megolm split, 3-arg + batch `encryptOlm`, `decryptOlm` → `Result<PlaintextOlmEvent<*>>`, `OlmEncryptionServiceImpl(driver)`) is exactly as mapped.
- **`model.rooms.GetEvents` → `model.room.GetEvents`** (the `rooms`→`room` singular rename applies here too — the `Direction.BACKWARDS` import breaks without it).
- **`MatrixClientAuthProviderData` lives in `de.connect2x.trixnity.clientserverapi.client`** (not `…trixnity.client`); `classicLogin` is a top-level companion extension in the same package.
- **`-Xcontext-parameters` compiler flag is REQUIRED** (server module, `freeCompilerArgs`): v5's Room repositories (`RoomStateRepository.get`, `RoomUserReceiptsRepository.get`) and `OlmCryptoStore.updateOutboundMegolmSession` are declared with Kotlin context parameters (`context(ReadTransaction)` / `context(StoreWriteTransaction)`). Consequences beyond the flag: `rotateStaleMegolmOnce` must wrap the `updateOutboundMegolmSession` call in `writeTransaction { }`, and helpers called from inside `readTransaction` that touch contextual repos must declare their own `context(transaction: ReadTransaction)` (`bridgeBotOf` did).
- **Proguard keep rule:** the JNA libolm wrapper moved to package `de.connect2x.trixnity.libolm` (not `…trixnity.olm`) — `-keep class de.connect2x.trixnity.libolm.** { *; }` in `server/proguard-rules.pro`.
- **Unread-count call sites:** the four `room.unreadMessageCount` reads are replaced by a per-room `unreadCounts[roomId.full]` map fed by `c.notification.getCount(roomId)` (import `de.connect2x.trixnity.client.notification` for the `MatrixClient.notification` extension); `notifyForEvent`/`seedRoomList`/`resolveRoomListEntry` read the map. Badges are now notification-counts, not message-counts (accepted UX change).
- **Line numbers:** MatrixRepository.kt is ~5.87k lines (not 6.4k); the map's pre-edit line references (e.g. :4527/:4677/:5030/:5102) point at the v4 state.
- **DB migration 4→9 execution-verified** (emulator, real schema-4 DB): `user_version` 4→9 on open; the built-in Account→Authentication credential migration moves `baseUrl`/`accessToken` into the new `Authentication` row while `userId`/`deviceId`/`syncBatchToken`/`filter`/`profile` stay on Account; 4 rooms / 38 timeline events preserved; session restored **without re-login** and the sync loop came up against the local Synapse dev homeserver.

...
**IMPLEMENTED 2026-09-05 (emulator-verified, uncommitted — see WORKLOG):** all three levers kept. Ingest gate = `yieldToSyncIngest()` (sync-round stamping in the /sync interceptor + `timedSyncOnce`, so both ACTIVE long-poll and SLOW syncOnce rounds gate it; the slow-mode "round already ended" bug was found and fixed during verification); `syncOnceFilter` timeline limit 50 → 20; background `state` excluded (`notTypes = ["*"]`). Emulator: no regression on any scenario, deep 35-event truncation backlog caught up fully via Trixnity's limited-loop, invite delivery works from stripped invite_state, wake verification never missed.
**LP3-VERIFIED 2026-09-05/06 (chats-audit-2026-09-05/phase1-burst*, committed `add130c`):** the on-device story was **cold-start catchup starvation, not steady-state ingest** — the gate's original 8 s yield cap let consumers barge in cycles and starved a 44-event catchup ingest for 175 s; cap now 60 s, retest burst 57 events in 2.8 s, steady rounds 39-99 ms. Same sessions fixed the SQLite pool exhaustion behind the 34 s send stall (chain-walk semaphore(2)), removed the composer ack hold (sends + edits now Beeper-style optimistic), and the WhatsApp-LID room-title regression (provision-contact name fallback). Details in WORKLOG 2026-09-05/06.
...
2. **Tighten the tool poll ticks** — the revision check is one tiny binder read; list 5 s → 2 s, thread 3 s → 1.5 s while screen-on. Cheap; no flow plumbing needed (tool-side receivers risk plugin-scan bans — not worth it for ≤2 s).
   **DONE 2026-09-05 (list 2 s `ChatListScreen.kt`, thread 1.5 s `ThreadScreen.kt`). Next step per the Molly-light comparison: delete the ticks entirely — long-poll `waitForRevision` over the binder.**
...
1. **A/B the 15-min lazy cadence** (0.8.4, shipped but never measured): one control night vs the 08-20 baseline (1.28%/h). If no measurable win and delivery gaps appear, revert to 5-min flat — simpler.
   **DONE 2026-09-05 — KEEP (WORKLOG):** control night 09-01→09-02 (`battery-audit-2026-09-02`, whole night on the new build) vs the 08-20 night: whole-device screen-off drain 1.08 → 0.93 %/h (−14 %; the 1.28 %/h baseline figure was whole-device, not per-uid), chats screen-off CPU 17m38s → 8m33s (−52 %), WiFi tx 2.43 → 0.52 MB (−79 %, the 3×-fewer-rounds signature), push wakeups unchanged (268 → 264 AP wakeups ≈ every 2.2 min all night — channel alive). No delivery-gap signals (no sync-pending notifications, no dead stretches in wake history). Caveat: the capture's app logcat is empty (verbose flag off) so the 900s intervals aren't positively confirmed — one verbose-flag night would close that.
2. **Durable push queue** (Beeper's `PushPayload` pattern, lite version): persist SSE-delivered pending pushes to a small file so a process death before catchup can't silently drop them (currently only ntfy `?since=` replay saves us). Bounded file, ids only.
   **DONE 2026-09-05 (WORKLOG):** `MatrixRepository` push queue — one JSON array in a `push_queue` SharedPreferences file (ids only, ≤50 entries, 12h age bound = ntfy's replay horizon). Real-message pushes enqueue on delivery (`onPushDelivered`, before the wake debounce); `runPushWake` clears it when caught up (or when the event is already in the store); `drainPushQueue()` at slow-sync engagement re-wakes the first missing event once per process after a restart (one syncOnce to the present covers the whole queue). Emulator-verified end to end incl. force-stop-between-delivery-and-catchup (push/README.md §3).
